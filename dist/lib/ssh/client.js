...
const tls = tslib_1.__importStar(require("node:tls"));
    ...
    createTLSConnection(timeout) {
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                reject(new errors_1.SSHTimeoutError('TLS connection timed out'));
            }, timeout);
            ...
            socket.on('error', (err) => {
                clearTimeout(timeoutId);
                reject(new errors_1.SSHTLSError(err.message, err));
            });
        });
    }
    /**
    ...
    performSSHHandshake(tlsSocket, timeout) {
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                client.end();
                tlsSocket.destroy();
                reject(new errors_1.SSHTimeoutError('SSH handshake timed out'));
            }, timeout);
            client.on('ready', () => {
                clearTimeout(timeoutId);
                resolve(client);
            ...
            client.on('error', (err) => {
                ...
                if (err.message.includes('authentication') || err.message.includes('Auth')) {
                    reject(new errors_1.SSHAuthenticationError(err.message, err));
                }
                else {
                    reject(new errors_1.SSHHandshakeError(err.message, err));
                }
            });
            client.on('close', () => {
                clearTimeout(timeoutId);
    ...
    async execute(command, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (options.signal?.aborted) {
                reject(new errors_1.SSHError('Operation aborted'));
                return;
            }
            const abortHandler = () => {
                reject(new errors_1.SSHError('Operation aborted'));
            ...
            client.exec(command, (err, stream) => {
                if (err) {
                    ...
                    reject(new errors_1.SSHError(`Failed to execute command: ${err.message}`, err));
                    return;
                }
                stream.on('data', (data) => {
                ...
                stream.on('error', (streamErr) => {
                    options.signal?.removeEventListener('abort', abortHandler);
                    reject(new errors_1.SSHError(`Stream error: ${streamErr.message}`, streamErr));
                });
            });
        });
    }
    ...
    async shell(options = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            client.shell({
                ...
                if (err) {
                    reject(new errors_1.SSHError(`Failed to open shell: ${err.message}`, err));
                    return;
                }
                let exitCode = 0;
    ...
    async upload(localPath, remotePath, options = {}) {
        ...
        if (stats.isDirectory()) {
            if (!options.recursive) {
                throw new errors_1.SFTPError('Source is a directory, use recursive option');
            }
    ...
    getSFTP(client) {
        return new Promise((resolve, reject) => {
            client.sftp((err, sftp) => {
                if (err) {
                    reject(new errors_1.SFTPError(`Failed to open SFTP: ${err.message}`, err));
                }
                else {
                    resolve(sftp);
                }
    ...
    sftpStat(sftp, remotePath) {
        return new Promise((resolve, reject) => {
            sftp.stat(remotePath, (err, stats) => {
                if (err) {
                    reject(new errors_1.SFTPError(`Failed to stat remote path: ${err.message}`, err));
                }
                else {
                    resolve(stats);
                }
    ...
    async uploadFile(sftp, localPath, remotePath, options) {
        ...
        return new Promise((resolve, reject) => {
            sftp.fastPut(localPath, remotePath, {
                ...
                if (err) {
                    reject(new errors_1.SFTPError(`Failed to upload file: ${err.message}`, err));
                }
                else {
    ...
    sftpMkdir(sftp, remotePath) {
        return new Promise((resolve, reject) => {
            sftp.mkdir(remotePath, (err) => {
                ...
                if (err && err.code !== 4) {
                    reject(new errors_1.SFTPError(`Failed to create directory: ${err.message}`, err));
                }
                else {
                    resolve();
                }
    ...
    async downloadFile(sftp, remotePath, localPath, options) {
        ...
        return new Promise((resolve, reject) => {
            sftp.fastGet(remotePath, localPath, {
                ...
                if (err) {
                    reject(new errors_1.SFTPError(`Failed to download file: ${err.message}`, err));
                }
                else {
                    resolve({ filesTransferred: 1, bytesTransferred: fileSize });
                }
    ...
    sftpReaddir(sftp, remotePath) {
        return new Promise((resolve, reject) => {
            sftp.readdir(remotePath, (err, list) => {
                if (err) {
                    reject(new errors_1.SFTPError(`Failed to read directory: ${err.message}`, err));
                }
                else {
                    resolve(list);
                }
