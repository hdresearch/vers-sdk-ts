...
export function init(params: {
  ...
  const client = params.client || new Vers({ versURL: readEnvOrError('VERS_VERS_URL') });
...
export async function executeHandler(
  ...
  try {
    ...
    console.error(`[MCP Server] Tool handler '${tool.name}' failed:`, {
      message: e.message,
      stack: e.stack,
      cause: e.cause,
    ...
    // Re-throw to let the MCP framework handle JSON-RPC response
    throw e;
  }
}
